...
**10. 指定建蔽率・指定容積率の取り違え疑い**
- 重説の指定建蔽率と指定容積率の数値を比較し、**建蔽率 > 容積率**（例: 建蔽60・容積40）の場合は、日本の用途地域では通常 建蔽率 ≦ 容積率 のため取り違えの可能性が高いです。
- status: "warning" で「指定建蔽率が指定容積率より大きいです（例: 60/40）。建蔽率と容積率が入れ替わっていないか確認してください（通常は建蔽率≦容積率。例: 建蔽40・容積60）。」と指摘してください。
- 0%表記ルールとは別に独立してチェックしてください。
...
**13. 排水施設（汚水・雑排水・雨水）の表記**
- ハトサポ／重説の選択肢は **「公共下水」** です（「公共下水道」ではありません）。
- 汚水・雑排水・雨水のいずれかに **「公共下水道」** と記載されている場合、status: "warning" で「『公共下水道』ではなく『公共下水』と記載してください（選択肢の正式名称）。」と指摘してください。
- 根拠資料側が「公共下水道」、重説側が「公共下水」の場合は**意味は同一**のため、不一致（error）として指摘しないでください。
...
**【重要：建蔽率と容積率の取り違え・表記】**
- 都市計画図等で『容積率／建蔽率』『60%／40%』のように並記されている場合、一般に左側＝容積率・右側＝建蔽率です。
- 重説上で**指定建蔽率 > 指定容積率**（例: 建蔽60・容積40）なら、取り違えの可能性として status: "warning" で指摘してください（通常は建蔽率 ≦ 容積率）。
- 根拠資料の左右と重説の割当が食い違う場合も、数値の単純不一致としてだけでなく取り違えの可能性に言及してください。

**【重要：排水施設表記の同一視】**
- 「公共下水」と「公共下水道」は意味上同一です。根拠資料と重説で表記が異なっても不一致（error）にしないでください。
- ただし重説側に「公共下水道」と書いてある場合は、ハトサポ選択肢に合わせて「公共下水」へ直すよう status: "warning" で指摘してください。
...
**【下水道図面の判定緩和ルール】**
地域の排水インフラの実態に合わせて、公共下水（公共下水道）が未整備の地域では「下水道」に関する図面の不足警告を出さないでください。
※重説の選択肢表記は「公共下水」です。「公共下水道」と書かれていても意味は同一として扱ってください。

**1. 排水設備の確認:**
- 重要事項説明書の「飲用、電気、ガスの供給施設及び排水施設の整備状況」の欄で、**排水（下水）**の整備状況を確認してください。
- 排水が「公共下水」「公共下水道」ではなく、「浄化槽」または「汲取り」と記載されている場合、あるいは「下水道：無」となっている場合は、**公共下水が未整備（浄化槽等）の地域**と判断してください。
...
9. **指定建蔽率 > 指定容積率** なら warning（取り違え疑い。通常は建蔽≦容積。例: 60/40 → 建蔽40・容積60を確認）
10. **北側斜線制限・日影規制** 空欄に斜線が入っていないなら warning
11. **⑪敷地と道路との関係図** 図または文章がなければ error
12. **汚水・雑排水・雨水** 「公共下水道」と記載されていれば warning（正式選択肢は「公共下水」。「公共下水」ならOK）
※添付書類の照合は別処理で行うため、本チェックでは上記まで
